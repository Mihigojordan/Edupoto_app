...
class ApiClient extends GetxService {
  ...
  final int timeoutInSeconds = 1500;
...
if ((GetPlatform.isWeb || '${deiceInfo.data['isPhysicalDevice']}' == 'true') || AppConstants.demo) {
  _mainHeaders!.addAll({
    'device-id': uniqueId,
    'os': GetPlatform.isWeb 
        ? 'web' 
        : GetPlatform.isAndroid 
            ? 'android' 
            : 'ios',
    'device-model': GetPlatform.isWeb
        ? '${deiceInfo.data['browserName'] ?? ''} ${deiceInfo.data['platform'] ?? ''}'.trim()
        : GetPlatform.isAndroid
            ? '${deiceInfo.data['brand'] ?? ''} ${deiceInfo.data['device-model'] ?? ''}'.trim()
            : '${deiceInfo.data['model'] ?? ''}'.trim(),
  });
}
 ...
 Future<Response> postData(String uri, dynamic body, {Map<String, String>? headers}) async {
  ...
  try {
    ...
    Future<Response> postMoneyData(
      String uri, dynamic body, {Map<String, String>? headers}) async {
      // Check if running on web and skip VPN checks
  if (!kIsWeb) {
    // Only check for VPN if not running on the web
    if (await ApiChecker.isVpnActive()) {
      return const Response(statusCode: -1, statusText: 'you are using vpn');
    }
  }{
