...
import 'package:hosomobile/features/transaction_money/domain/models/contact_tag_model.dart';
...
class StudentRegistrationController extends GetxController implements GetxService {
  final TransactionRepo transactionRepo;
  final StudentRegistrationRepo studentRegistrationRepo;
  final PaymentRepo paymentRepo;
  final AuthRepo authRepo;
  StudentRegistrationController({required this.transactionRepo, required this.authRepo,required this.paymentRepo, required this.studentRegistrationRepo});
...
>>>>>>> 70f2993a9c488529ef4a6b7bd31749fa3d235e6b
  ...
  //  Response responseMtn = await studentRegistrationRepo.sendMoneyMtnMomoApi(phoneNumber: contactModel.phoneNumber, amount: amount, purpose: purpose, pin: pinCode);
   
  //  if(responseMtn.statusCode == 202){

    Response response = await studentRegistrationRepo.addStudentApi(name: name, code: code, schoolCode: schoolCode, parentId: parentId, classId: classId); //,gender:gender!, age:age!,hipSize:hipSize!,waistSize:waistSize!,heightSize:heightSize!, topWear:topWear! ,bottomWear:bottomWear!,sportsWear:sportsWear!,feetWear:feetWear!,topSize:topWear,size:size! );
   ...
   return response;
     ...
     Future<Response> updateStudent({required String name,required String code,required String schoolCode,required String parentId,required int classId,required int id,required String gender,required String age,required String hipSize,required String waistSize,required String heightSize,required String topWear,required String bottomWear,required String sportsWear,required String feetWear,required String topSize,required String shoeSize,required String sportSize,required String shoulderSize,required String handSize }) async{
  ...
  //  if(responseMtn.statusCode == 202){
    ...
    Response response = await studentRegistrationRepo.updateStudentApi(name: name, code: code, schoolCode: schoolCode, parentId: parentId, classId: classId,id:id,gender:gender, age:age,hipSize:hipSize,waistSize:waistSize,heightSize:heightSize, topWear:topWear ,bottomWear:bottomWear,sportsWear:sportsWear,feetWear:feetWear,topSize:topSize,shoeSize:shoeSize,sportSize:sportSize,shoulderSize:shoulderSize,handSize:handSize);
    if(response.statusCode==200){
        _isLoading = false;
     _isNextBottomSheet = true;
   ApiChecker.checkApi(response);
    ...
    // }
     ...
     _isLoading = false;
     ApiChecker.checkApi(response);
     print('111111111111111111111111111111111111111111${response.body}');
   }
   update();
   return response;
  }
  ...
  //     Get.find<StudentRegistrationController>().checkAgentNumber(phoneNumber: phoneNumber).then((value) {
  //       if(value.isOk){
  ...
  //     });
  //   }else{
  //     Get.find<StudentRegistrationController>().checkCustomerNumber(phoneNumber: phoneNumber).then((value) {
  //       if(value!.isOk){
